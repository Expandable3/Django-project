...
# uuidField = uuid.UUID(str(uuidField))
...
class Bookinstances(models.Model):
    ...
    book1=models.ForeignKey('Book',on_delete=models.SET_NULL,null=True)
    print("sahil2121212121212")
    print(book1)
    ...
    class Meta:
        permissions=(("can_mark_returned","set book as returned "),("staff","for staff only"),)
        # ("can_renew","renew the book"),
        ordering=['id']
    ...
    def __str__(self):  
        print(self.id)
        return ('{0} ({1})'.format(self.id, self.book1.title))
